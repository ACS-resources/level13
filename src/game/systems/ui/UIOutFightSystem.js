define([
	...
	var UIOutFightSystem = Ash.System.extend({
		...
		updateFightActive: function () {
			...
			$(		"#fight-popup-self-name").text(this.numExplorers > 0 ? " 队伍 " : " 流浪者 ");
		...
		getDescriptionByContext: function (context, enemy) {
			var enemyNoun = TextConstants.getEnemyNoun([enemy], false, false);
									...
									switch (baseActionID) {
										case "scavenge":
											return "搜索时被" + Text.addArticle(enemyNoun) + "袭击";
										case "investigate":
											return "调查时被" + Text.addArticle(enemyNoun) + "袭击";
										case "scout_locale_u":
											return "侦察时被" + Text.addArticle(enemyNoun) + "袭击";
										case "scout_locale_i":
											return "侦察时遇袭";
										case "clear_workshop":
											 var enemyActiveV = TextConstants.getEnemyActiveVerb([ enemy ]);
											return "工坊被" + Text.pluralify(enemyNoun) + enemyActiveV;
										case "fight_gang":
											return Text.addArticle(enemyNoun) + "挡住了去路";
										case "wait":
											return "被" + Text.addArticle(enemyNoun) + "袭击";
										default:
											return Text.addArticle(enemyNoun) + "逼近了";
									}
		},
		...
		getStatsText: function (att, def, speed, hp, shield) {
			...
			result += "防御: " + def;
			result += " | ";
			result += "速度: " + Math.round(speed * 20)/20;
			result += " | ";
