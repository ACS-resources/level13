define([
	...
	var UIOutTradeSystem = Ash.System.extend({
			...
			if (this.availableTradingPartnersCount > 0) {
				...
				if (totalCaravans > 0) {
					$("#trade-caravans-outgoing-num").html(
						"可用商队: <span class='hl-functionality'>" + numAvailableCaravans + "/" + totalCaravans + "</span>. " +
							"容量: <span class='hl-functionality'>" + this.getCaravanCapacity() + "</span> 每个商队.");
				} else {
					var stableUnlocked = GameGlobals.playerActionsHelper.isRequirementsMet("build_in_stable");
					if (stableUnlocked) {
						$("#trade-caravans-outgoing-num").html("可用商队: <span class='hl-functionality'>0</span> (建造马厩以派出商队)");
					} else {
						$("#trade-caravans-outgoing-num").html("");
					}
				}
				...
				for (let i = 0; i < busyCaravans.length; i++) {
					...
					li += " - ";
					li += "<span class='sell'>卖出: " + caravan.sellGood + " x" + caravan.sellAmount + "</span>";
					li += "<span class='buy'>获得: " + caravan.buyGood + " x" + buyAmount + "</span>";
					li += "<span class='return-time'>返回时间: " + timeUntilString + "</span>";
					li += "</li>";
		...
		hideOutgoingPlanRows: function () {
			$(".btn-trade-caravans-outgoing-toggle").text("派出商队");
		...
		showOutgoingPlanRow: function (tradePartnerOrdinal) {
			...
			$("#trade-caravans-outgoing-" + tradePartnerOrdinal + " button").text("取消");
		...
		updateIncomingCaravan: function (isActive) {
			...
			if (caravan) {
				...
				var buttonsTD = "<td class='nowrap-on-regular-layout'><button class='trade-caravans-incoming-trade'>交易</button>";
				buttonsTD += "<button class='trade-caravans-incoming-dismiss btn-secondary'>送走</button></td>";
		...
		updateOutgoingCaravanPrepare: function () {
			...
			if (hasEnoughSellRes) {
				...
				$(trID + " .trade-sell-value-invalid").text(selectedSell + "不足");
