define([
	...
	var UIOutMapSystem = Ash.System.extend({
		...
		initMapModeSelector: function () {
			...
			var html = "";
			html += "<option value='" + MapUtils.MAP_MODE_DEFAULT + "' id='map-style-selector-" + this.MAP_MODE_DEFAULT + "'>默认</option>";
			html += "<option value='" + MapUtils.MAP_MODE_HAZARDS + "' id='map-style-selector-" + this.MAP_MODE_HAZARDS + "'>危险</option>";
			html += "<option value='" + MapUtils.MAP_MODE_SCAVENGING + "' id='map-style-selector-" + this.MAP_MODE_SCAVENGING + "'>搜索</option>";
		...
		getLevelSelectorOptionLabel: function (level, isCleared) {
				...
				if (level == surfaceLevel) result = "地表";
				if (level == groundLevel) result = "地面";

				if (isCleared) result += " (已清除)";
				else result += " (未清除)";
		...
		showSectorPath: function () {
			...
			let title = "从 " + startPosition.getInGameFormat(includeLevel) + " 到 " + position.getInGameFormat(includeLevel) + "的路线";
		...
		getDistrictText: function (sector) {
			let sectorFeatures = sector.get(SectorFeaturesComponent);
			return sectorFeatures.isEarlyZone() ? "中心" : "外围";
		},
		...
		getPOIText: function (sector, isScouted) {
			...
			let result = [];
			if (sector.has(CampComponent)) result.push("营地");
			...
			if (improvements.getCount(improvementNames.greenhouse)) result.push("温室");
			if (!hasCampOnLevel && sectorFeatures.canHaveCamp()) result.push("适合建造营地");
			...
			if (unScoutedLocales > 0) result.push("未探索地点");
			if (numUnexaminedSpots > 0) result.push("未调查特征");
		...
		getResScaText: function (sector, isScouted, statusComponent, featuresComponent) {
			...
			if (featuresComponent.heapResource) {
				...
				result += ", " + heapName + " (" + resourceName + ", 已搜刮" + heapScavengedPercentage + "%)";
			}
			
			if (investigatedPercent > 0) {
				result += " (已调查" + investigatedPercent + "%) ";
			}
		...
		getCollectorsText: function (sector, isScouted) {
			...
			if (isScouted) {
				...
				if (collectorFood.count == 0 && featuresComponent.resourcesCollectable.food > 0 && hazards.territory == 0) result2.push ("食物")
				if (collectorWater.count === 1) result1.push("1个水桶");
				if (collectorWater.count > 1) result1.push("水桶");
				if (collectorWater.count == 0 && featuresComponent.resourcesCollectable.water > 0 && hazards.territory == 0) result2.push ("水")
			}
			...
			let part2 = "";
			if (result2.length > 0) part2 = " (可收集" + result2.join(", ") + ")";
			return part1 + part2;
		},
		...
		getEnvironmentHTML: function (sector, isScouted) {
			...
			if (hazards.debris > 0) result.push("废墟");
			if (hazards.territory > 0) result.push("帮派地盘");
			
			if (hazards.radiation > 0)
				result.push(getHazardSpan("辐射", hazards.radiation, hazards.radiation > itemsComponent.getCurrentBonus(ItemConstants.itemBonusTypes.res_radiation)));
			if (hazards.poison > 0)
				result.push(getHazardSpan("污染", hazards.poison, hazards.poison > itemsComponent.getCurrentBonus(ItemConstants.itemBonusTypes.res_poison)));
			if (hazards.cold > 0)
				result.push(getHazardSpan("寒冷", hazards.cold, hazards.cold > itemsComponent.getCurrentBonus(ItemConstants.itemBonusTypes.res_cold)));
			if (hazards.flooded > 0)
				result.push(getHazardSpan("积水", hazards.flooded, hazards.flooded > itemsComponent.getCurrentBonus(ItemConstants.itemBonusTypes.res_water)));
		...
		getMiscHTML: function (sector, isScouted) {
			...
			if (isScouted) {
				...
				if (statusComponent.graffiti) {
					result.push("涂鸦: '" + statusComponent.graffiti + "'");
				}
			}
