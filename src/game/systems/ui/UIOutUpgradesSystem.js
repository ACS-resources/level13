	...
	let UIOutUpgradesSystem = Ash.System.extend({
		...
		refreshTechDetails: function () {
			...
			if (hasSelection) {
				...
				var statusS = isUnlocked ? "已研究" : isAvailable ? "可用" : "锁定";
		...
		getUpgradeTR: function (upgradeDefinition, status) {
			...
			switch (status) {
				...
				case UpgradeConstants.upgradeStatus.BLUEPRINT_IN_PROGRESS:
					...
					buttonTD = "<td class='list-action'><button class='action multiline' action='create_blueprint_" + upgradeDefinition.id + "'>合成</button></td>";
					break;
				default:
					buttonTD = "<td></td>";
					break;
		...
		getUnlockedResearchDescription: function (upgradeID) {
			...
			switch (status) {
				...
				default:
					...
					return "解锁研究: " + shownResearchNames.join(", ");
			}
		},
