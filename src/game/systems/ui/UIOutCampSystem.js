	...
	let UIOutCampSystem = Ash.System.extend({
		...
		updatePopulationDisplayFast: function () {
			...
			if (populationChangePerSecWithoutCooldown > 0) {
				...
				if (hint) {
						this.elements.populationDecreaseHint.text("人们正在离开，原因是：" + hint);
					} else {
						this.elements.populationDecreaseHint.text("人们正在离开，原因是：声誉过低");
					}
			}
		...
		initImprovements: function () {
			...
			for (let i = 0; i < improvementIDs.length; i++) {
				...
				let repairButton = "<button class='action action-repair btn-narrow' data-tab='switch-in' action='repair_in_" + key + "'>修复</button>";
				let damagedIcon = "<img src='img/eldorado/icon-gear-warning.png' class='icon-damaged icon-ui-generic icon-centered' alt='建筑已损坏' title='建筑已损坏' />"
			...
			let ths = "<tr class='header-mini'><th></th><th>数量</th><th>等级</th><th></th><th></th><th></th></tr>"
			$table.append(ths);
			$table.append(trs);
		...
		initWorkers: function () {
			...
			for (let key in CampConstants.workerTypes) {
				...
				tds += "<td class='in-assign-worker-auto'><input type='checkbox' id='in-assing-worker-auto-" + def.id + "' class='in-assign-workers-auto-toggle' title='自动分配工人' /></td>"
		...
		updateImprovements: function () {
			...
			for (let i = 0; i < this.elements.improvementRows.length; i++) {
				...
				if (isDamaged) {
					...
					let damageDescription = "建筑已损坏";
					if (damagedSource) {
						damageDescription += " 由 " + damagedSource;
					}
					if (numBuilt > 1) {
			...
			if (isActive && showEvents) {
				...
				UIState.refreshState(this, "outgoing-caravans-num", numCaravans, function () {
												...
												for (let i = 0; i < numCaravans; i++) {
														...
														bar += '<span class="progress progress-label">外出商队</span>';
														bar += '</div>';
		...
		getCampMiscEventsData: function () {
			...
			for (let i = 0; i < campComponent.disabledPopulation.length; i++) {
				...
				if (pop.num > 0) {
					...
					let label = "停工工人 (" + pop.num + ")";
					if (pop.reason == CampConstants.DISABLED_POPULATION_REASON_DISEASE) label = "疾病 (" + pop.num + ")";
					let isNegative = true;
		...
		updateStats: function () {
			...
			if (showLevelStats) {
				...
				if (hasUnlockedTrade) {
					...
					$("#in-demographics-trade-network .value").text(hasAccessToTradeNetwork ? "是" : "否");
			...
			if (GameConstants.isDebugVersion) {
				...
				for (let key in OccurrenceConstants.campOccurrenceTypes) {
					...
					if (campTimers.eventStartTimers[event]) {
						debugInfoText += "下次" + event + "还需" + UIConstants.getTimeToNum(campTimers.eventStartTimers[event]) + "<br/>";
					}
					if (campTimers.eventEndTimers[event] && campTimers.eventEndTimers[event] != OccurrenceConstants.EVENT_DURATION_INFINITE) {
						debugInfoText += event + "结束还需" + UIConstants.getTimeToNum(campTimers.eventEndTimers[event]) + "<br/>";
					}
				}
		...
		getWorkerDescription: function (def) {
			...
			switch (def.id) {
				...
				case CampConstants.workerTypes.gardener.id:
						productionS = "草药 +" + UIConstants.roundValue(GameGlobals.campHelper.getHerbsProductionPerSecond(1, improvements), true, true) + "/s";
						break;
					case CampConstants.workerTypes.apothecary.id:
						...
						specialConsumptionS = "<br/><span class='warning'>草药 -" + GameGlobals.campHelper.getWorkerHerbsConsumptionPerSecond(1) + "/s</span>";
					break;
				...
				case CampConstants.workerTypes.robotmaker.id:
						...
						productionS = "机器人 +" + UIConstants.roundValue(robotVal, true, true, robotValDivisor) + "/s";
						specialConsumptionS = "<br/><span class='warning'>工具 -" + GameGlobals.campHelper.getToolsConsumptionPerSecondRobots(1) + "/s</span>";
					break;
				case CampConstants.workerTypes.scientist.id:
						productionS = "证据 +" + UIConstants.roundValue(GameGlobals.campHelper.getEvidenceProductionPerSecond(1, improvements), true, true, 1000) + "/s";
						break;
					case CampConstants.workerTypes.cleric.id:
						productionS = "希望 +" + UIConstants.roundValue(GameGlobals.campHelper.getHopeProductionPerSecond(1, improvements), true, true, 100000) + "/s";
						break;
		...
		getRaidDangerCalloutContent: function () {
			...
			let result = Text.t("ui.camp.raid_danger_description");
				result += "<hr/>";
				result += "危险点数: " + dangerPoints + "<br/>";
				result += "防御点数: " + defencePoints + "<br/>";
		...
		getDiseaseChanceCalloutContent: function () {
			...
			let result = "营地中出现的疾病转变为爆发的风险";
				result += "<hr/>";
				result += "人口因素: " + Math.round(OccurrenceConstants.getDiseaseOutbreakChance(campComponent.population, false, false, 0) * 100) + "%<br/>";

			if (hasMedicine) {
				result += "药物: -" + ((1 - OccurrenceConstants.getDiseaseMedicineFactor(hasMedicine, apothecaryLevel)) * 100) + "%<br/>";
			} else if (hasHerbs) {
				result += "草药: -" + ((1 - OccurrenceConstants.getDiseaseHerbsFactor()) * 100) + "%<br/>";
			}
