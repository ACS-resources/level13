define([
	...
	let UIOutHeaderSystem = Ash.System.extend({
		...
		SCAVENGE_BONUS_TYPES: [
			{ itemBonusType: ItemConstants.itemBonusTypes.scavenge_general, displayName: "常规", containerID: "scavenge-bonus-general" },
			{ itemBonusType: ItemConstants.itemBonusTypes.scavenge_ingredients, displayName: "材料", containerID: "scavenge-bonus-ingredients" },
			{ itemBonusType: ItemConstants.itemBonusTypes.scavenge_supplies, displayName: "补给", containerID: "scavenge-bonus-supplies" },
		],
		...
		updatePlayerStats: function () {
			...
			this.updateStatsCallout("让探索更安全，搜刮更高效", this.elements.statIndicatorVision, playerStatsNode.vision.accSources);
			...
			this.updateHealthStatCallout("决定最大耐力值", this.elements.statIndicatorHealth);
			...
			this.updateStatsCallout("探索所需", this.elements.statIndicatorStamina, playerStatsNode.stamina.accSources);
			...
			if (isInCamp && campComponent && reputationComponent && maxPopulation > 0) {
				...
				var populationCalloutContent = "所需声望:<br/>";
		...
		refreshStatuses: function () {
			...
			if (GameGlobals.playerHelper.getCurrentBonus(ItemConstants.itemBonusTypes.detect_hazards) > 0) {
				statuses.push({ name: "危险预警", icon: "img/status-hazard-prediction.png", isNegative: false });
			}
			
			if (GameGlobals.playerHelper.getCurrentBonus(ItemConstants.itemBonusTypes.detect_supplies) > 0) {
				statuses.push({ name: "补给探测", icon: "img/status-supplies-prediction.png", isNegative: false });
			}
			
			if (GameGlobals.playerHelper.getCurrentBonus(ItemConstants.itemBonusTypes.detect_ingredients) > 0) {
				statuses.push({ name: "材料探测", icon: "img/status-ingredients-prediction.png", isNegative: false });
			}
			
			if (GameGlobals.playerHelper.getCurrentBonus(ItemConstants.itemBonusTypes.detect_poi) > 0) {
				statuses.push({ name: "地点探测", icon: "img/status-poi-prediction.png", isNegative: false });
			}
		...
		getLevelIcon: function (inCamp, sector) {
			...
			if (inCamp) {
				...
				desc = levelComponent.habitability < 1 ? "营地 | 前哨站" : "营地 | 常规";
