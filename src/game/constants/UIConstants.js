...
define(['ash',
	...
	var UIConstants = {
		...
		FEATURE_MISSING_TITLE: "功能未实现",
		FEATURE_MISSING_COPY: "此功能尚未实现, 请稍后再来! ",
		...
		names: {
			resources: {
				...
				resource_food: "食物",
				resource_water: "水",
				resource_concrete: "混凝土",
				resource_herbs: "草药",
				resource_medicine: "药品",
				resource_tools: "工具",
				resource_robots: "机器人",
				item_exploration_1: "开锁器",
				rumours: "传闻",
				evidence: "证据",
				insight: "洞察力",
			}
		...
		getItemCallout: function (item, smallCallout, showBagOptions, bagOptions, tab) {
			...
			let itemName = ItemConstants.getItemDisplayName(item);
			var itemCalloutContent = "<b>" + itemName + "</b><br/>类型: " + ItemConstants.getItemTypeDisplayName(item.type, false) + " " + detail;
			...
			if (ItemConstants.hasItemTypeQualityLevels(item.type)) {
				...
				itemCalloutContent += "</br>品质: " + ItemConstants.getQualityDisplayName(quality);
			}
			if (item.broken) itemCalloutContent += "<br><span class='warning'>已损坏</span>";
		...
		getExplorerDivWithOptions: function (explorerVO, isRecruited, isInCamp, questTextKey, isForced) {
			...
			if (!isInCamp && explorerVO.injuredTimer > 0) div += "<button class='action btn-narrow btn-heal-explorer' action='" + healAction + "' style='display:none'>治疗</button>";
			div += "</div>";
		...
		getExplorerCallout: function (explorer, isRecruited, isInCamp, hideButtons, questTextKey, isForced) {
			...
			if (isRecruited) {
				result += "<br/>队伍中: " + (explorer.inParty ? "是" : "否");
			}
			result += "<br/>类型: " + ExplorerConstants.getExplorerTypeDisplayName(explorerType);
			result += "<br/>能力: " + Text.t(ExplorerConstants.getAbilityTypeDisplayNameKey(explorer.abilityType))
				+ " (" + Text.t(UIConstants.getExplorerAbilityDescriptionTextVO(explorer, [])) + ")";

			if (questTextKey) {
				result += "<br/>任务: " + Text.t(questTextKey);
			}

			if (explorer.injuredTimer >= 0) {
				if (explorer.inParty) {
					result += "<br/>状态: " + this.warning("受伤");
				} else {
					result += "<br/>状态: " + this.warning("受伤 (恢复中) ");
				}
			...
			if (GameConstants.isCheatsEnabled) {
				result += "<br/>" + "<span class='debug-info'>" + this.meta("信任度: " + explorer.trust) + "</span>";;
			}
			...
			if (isRecruited && isInCamp && !hideButtons) {
				...
				options += makeButton("dismiss_explorer_" + explorer.id, "解散");
				if (!explorer.inParty) {
					options += makeButton("select_explorer_" + explorer.id, "加入队伍");
				} else {
					options += makeButton("deselect_explorer_" + explorer.id, "移出队伍");
				}
				options += "</div>";
				result += options;
			}
		...
		getCurrencyLi: function (amount, simple) {
			...
			div += "<div class='info-callout-target info-callout-target-small' description='银币'>";
		...
		getItemBonusName: function (bonusType, short) {
			switch (bonusType) {
				...
				case ItemConstants.itemBonusTypes.fight_shield: return "盾牌";
				case ItemConstants.itemBonusTypes.fight_speed: return "攻击速度";
				case ItemConstants.itemBonusTypes.movement: return "移动消耗";
				case ItemConstants.itemBonusTypes.scavenge_cost: return "搜索消耗";
				case ItemConstants.itemBonusTypes.scavenge_general: return "搜索加成";
				case ItemConstants.itemBonusTypes.scavenge_supplies: return "搜索加成";
				case ItemConstants.itemBonusTypes.scavenge_ingredients: return "搜索加成";
				case ItemConstants.itemBonusTypes.scavenge_blueprints: return "搜索加成";
				case ItemConstants.itemBonusTypes.scavenge_valuables: return "搜索加成";
				case ItemConstants.itemBonusTypes.scout_cost: return "侦查消耗";
				case ItemConstants.itemBonusTypes.collector_cost: return "陷阱/水桶消耗";
				case ItemConstants.itemBonusTypes.bag: return "背包容量";
				case ItemConstants.itemBonusTypes.res_cold: return "保暖";
				case ItemConstants.itemBonusTypes.res_radiation: return short ? "辐射防护" : "辐射防护";
				case ItemConstants.itemBonusTypes.res_poison: return short ? "中毒防护" : "中毒防护";
				case ItemConstants.itemBonusTypes.res_water: return short ? "防水" : "防水";
				case ItemConstants.itemBonusTypes.shade: return short ? "防光" : "防光盲";
				case ItemConstants.itemBonusTypes.detect_hazards: return short ? "危险探测" : "调查 (危险) ";
				case ItemConstants.itemBonusTypes.detect_supplies: return short ? "补给探测" : "调查 (补给) ";
				case ItemConstants.itemBonusTypes.detect_ingredients: return short ? "材料探测" : "调查 (材料) ";
				case ItemConstants.itemBonusTypes.detect_poi: return short ? "地点探测" : "调查 (地点) ";
				default:
		...
		getPerkTimerText: function (perk, isResting) {
			if (perk.removeTimer >= 0) {
				...
				return "完全生效时间: " + this.getTimeToNum(perk.startTimer);
			} else {
				return null;
			}
		},
		...
		getPerkBonusText: function (perk) {
			...
			switch (perk.type) {
				...
				case PerkConstants.perkTypes.luck:
					if (perk.effect > 0) {
						return "降低探索时负面随机事件的概率";
					} else { 
						return "提高探索时负面随机事件的概率";
					}
			}
		...
		getCostsSpans: function (action, costs) {
			...
			if (hasCosts) {
				...
				result += "<span class='action-cost p-meta'>免费</span><br />";
			}
			return result;
		},
	...
	getTimeSinceText: function (date) {
		...
		switch (featureID) {
			case UIConstants.UNLOCKABLE_FEATURE_MAP_MODES: return "地图模式";
			case UIConstants.UNLOCKABLE_FEATURE_WORKER_AUTO_ASSIGNMENT: return "工人自动分配";
		}
		return featureID;
	},

		getCampDisplayName: function (campNode, short) {
		return "第" + campNode.position.level + "层营地";
	},
