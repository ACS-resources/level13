...
function (Ash, ItemData, Text, MathUtils, PlayerActionConstants, ItemVO) {
	...
	let ItemConstants = {
		...
		getQualityDisplayName: function (quality) {
			if (quality === 'common') return '普通';
			if (quality === 'rare') return '稀有';
			if (quality === 'epic') return '史诗';
			if (quality === 'legendary') return '传说';
			return quality;
		},
		...
		getUseItemVerb: function (item) {
			...
			if (item.id.startsWith("cache_hope")) return "捐赠";
			if (item.id.startsWith("cache_robots")) return "修复";
			if (item.id.startsWith("robot")) return "修复";
			if (item.id.startsWith("document")) return "阅读";
			return "使用";
		},
