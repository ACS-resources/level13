	...
	function (Ash, DescriptionMapper, Text, TextBuilder, GameConstants, EnemyConstants, ItemConstants, SectorConstants, PositionConstants, MovementConstants, TradeConstants, WorldConstants) {
		...
		var TextConstants = {
			...
			getSectorTextParams: function (features) {
				...
				addOptions("a-building", ["顶天立地的", "高的", "昏暗的", "废弃的", "不可名状的", "小的", "常见的", "庞大的", "巨型的", "巨大的", "高大的", "宏伟的"]);
      ...
      getLocaleName: function (locale, sectorFeatures, isShort) {
        ...
        switch (locale.type) {
          ...
          case localeTypes.house:
            if (condition === SectorConstants.SECTOR_CONDITION_DAMAGED) modifier = "被摧毁的";
        ...
        getScaResourcesString: function (discoveredResources, knownResources, resourcesScavengable) {
          ...
          for (var key in resourceNames) {
            ...
            if (amount > 0 && discoveredResources.indexOf(name) >= 0) {
              var amountDesc = "稀有的";
              if (amount == WorldConstants.resourcePrevalence.RARE) amountDesc = "罕见的";
              if (amount == WorldConstants.resourcePrevalence.DEFAULT) amountDesc = "稀有的";
              if (amount == WorldConstants.resourcePrevalence.COMMON) amountDesc = "常见的";
              if (amount == WorldConstants.resourcePrevalence.ABUNDANT) amountDesc = "丰富的";
        ...
        getMovementBlockerAction: function (blockerVO, enemiesComponent, gangComponent) {
          switch (blockerVO.type) {
            case MovementConstants.BLOCKER_TYPE_GAP: return "桥接缺口";
        ...
        getUnblockedVerb: function (blockerType) {
          switch (blockerType) {
            case MovementConstants.BLOCKER_TYPE_GAP: return "已桥接";
      ...
      DescriptionMapper.add("sector-vision", { sectorType: wildcard }, "[a] [n-street] 在看起来像是 [a] [a-building] [n-building] 前");
      DescriptionMapper.add("sector-vision", { sectorType: wildcard }, "[a] [a-street] [n-street] 位于两个 [a-building] [n-buildings] 之间");
      DescriptionMapper.add("sector-vision", { sectorType: wildcard }, "[a] [a-street] [n-street] 位于两个 [n-buildings] 之间，两边各有一些 [a-building] [n-buildings]");
      DescriptionMapper.add("sector-vision", { sectorType: wildcard }, "[a] [a-street] [n-street] 与少数 [a-building] [n-buildings] 一同出现");
      DescriptionMapper.add("sector-vision", { sectorType: wildcard }, "[a] [a-street] [n-sector] 散布着 [an-items] 和 [an-items]");
      DescriptionMapper.add("sector-vision", { sectorType: wildcard }, "[a] [a-street] [n-street] 两旁是 [a-building] [n-buildings]");
      DescriptionMapper.add("sector-vision", { sectorType: wildcard }, "[a] [a-street] [n-street] 被 [n-buildings] 包围");
      DescriptionMapper.add("sector-vision", { sectorType: wildcard }, "[a] [a-street] [n-street] 被 [a-building] [n-buildings] 包围");
      DescriptionMapper.add("sector-vision", { sectorType: wildcard }, "[a] [n-street] 带有一些 [an-decos] 和 [a-building] [n-buildings]");
      DescriptionMapper.add("sector-vision", { sectorType: wildcard }, "[a] [a-street] [n-street] 位于一些 [n-buildings] 之间");
      DescriptionMapper.add("sector-vision", { isSurfaceLevel: false }, "[a] [n-street] 位于一个巨大支柱的底部，该支柱支撑着上面的层级");
      DescriptionMapper.add("sector-vision", { isSurfaceLevel: false, wear: b12, sunlit: false, debris: b0 }, "[a] [a-street] [n-street] 长期无人居住的建筑覆盖着奇异的苔藓");
      DescriptionMapper.add("sector-vision", { buildingDensity: b0, isGroundLevel: false }, "连接几栋建筑的桥梁和通道系统围绕着令人眩晕的开口至下层");
      DescriptionMapper.add("sector-vision", { buildingDensity: b12, isGroundLevel: false, campable: false }, "[a] [a-street] 桥梁跨越下方层级，为有轨电车轨道、公用设施和行人设置了不同的层级");
      DescriptionMapper.add("sector-vision", { buildingDensity: b22 }, "某种 [a] [a-sectortype] 复合体，有几条狭窄的通道");
      DescriptionMapper.add("sector-vision", { buildingDensity: b13 }, "一个宽阔的广场，一侧是 [a] [a-building] [n-building]，另一侧似乎是 [a] [a-building] [n-building] 的遗迹");
      DescriptionMapper.add("sector-vision", { buildingDensity: b23, isSurfaceLevel: false, sunlit: false }, "[a] [a-street] [n-street] 在一座巨大的 [n-building] 下方");
      DescriptionMapper.add("sector-vision", { buildingDensity: b23, isSurfaceLevel: false }, "[a] [n-street] 的多层通道沿着周围 [a-sectortype] 建筑的墙壁爬行");
      DescriptionMapper.add("sector-vision", { buildingDensity: b33 }, "某种 [a] [a-sectortype] 走廊位于两座巨大 [n-buildings] 之间，几乎没有足够的空间行走");
      DescriptionMapper.add("sector-vision", { buildingDensity: b33 }, "[a] [a-street] [n-street] 挤满了 [a-building] [n-buildings] 和 [an-decos]，几乎没有足够的空间通过");
      DescriptionMapper.add("sector-vision", { buildingDensity: b33 }, "[a] [a-street] 巷子位于两个 [a-building] [n-buildings] 之间");
      DescriptionMapper.add("sector-vision", { wear: b13, sunlit: false, level: lmodern, debris: b0 }, "[a] [a-street] [n-street] 两旁是高大的 [n-buildings]，排列着枯萎的树木，这些树木直到不久前还必须在人造光中茁壮成长");
      ...
      DescriptionMapper.add("sector-vision", { wear: b33 }, "[a] [a-building] 建筑，很难确定其原始用途，被剥离至裸露的混凝土");
      DescriptionMapper.add("sector-vision", { buildingDensity: b22, wear: b33 }, "[a] [a-street] 走廊，散布着长时间居民留下的垃圾");
      DescriptionMapper.add("sector-vision", { wear: b33, isSurfaceLevel: false }, "[a] [a-street] [a-sectortype] [n-street]，几座巨大的无法识别的废墟在其上方隐约可见");
      ...
      DescriptionMapper.add("sector-vision", { sectorType: t_I, buildingDensity: b23 }, "[a] [n-street] 位于看起来像是 [a-building] 控制室和办公室的两栋建筑之间");
      DescriptionMapper.add("sector-vision", { sectorType: t_M }, "[a] [a-street] [n-street] 在 [a] [n-building] 后面，低矮的天花板交错着旧电线和管道");
      ...
      DescriptionMapper.add("sector-vision", { sectorType: t_C }, "[a] [a-street] 购物街，遗留着各种商店和咖啡馆的残骸");
      ...
      DescriptionMapper.add("sector-vision", { sectorType: t_C, buildingDensity: b13 }, "[a] 宽阔的围栏露台附着在一座巨大的塔楼上，俯瞰下方的 [a-street] 街道");
      DescriptionMapper.add("sector-vision", { sectorType: t_C, buildingDensity: b13 }, "一个由 [a-building] 办公楼包围的圆形庭院");
      DescriptionMapper.add("sector-vision", { sectorType: t_C, buildingDensity: b22, wear: b33 }, "[a] [a-building] 建筑，很难确定其原始用途，被剥离至混凝土，中间有一个令人印象深刻的螺旋楼梯");
      DescriptionMapper.add("sector-vision", { sectorType: t_P }, "[a] [n-street]，由看起来曾经是某种公共设施的巨大建筑主导");
      DescriptionMapper.add("sector-vision", { sectorType: t_P }, "一段被一些较小建筑包围的废弃高速公路");
      DescriptionMapper.add("sector-vision", { sectorType: t_P, buildingDensity: b12 }, "[a] [a-street] [n-street] 主导着一排庄严的雕像");
      ...
      DescriptionMapper.add("sector-vision", { sectorType: t_P, buildingDensity: b33 }, "[a] [a-street] [n-street] 位于两座巨大 [n-buildings] 之间，几乎没有足够的空间通过");
      DescriptionMapper.add("sector-vision", { sectorType: t_S, buildingDensity: b33, wear: b22 }, "[a] [a-street] [n-street] 被 [a-building] 住宅包围（部分覆盖），这些建筑已经被遗弃了一段时间");
      ...
      DescriptionMapper.add("sector-vision", { level: 14, buildingDensity: b23 }, "[a] [a-street] 通道位于两个失效的、被墙壁封闭的核反应堆之间");
      DescriptionMapper.add("sector-vision", { level: 14, buildingDensity: b23 }, "[a] [a-street] [n-street] 在一个巨大的工业加工综合体外，所有入口都紧紧关闭");
      DescriptionMapper.add("sector-vision", { level: 14, buildingDensity: b33 }, "[a] [a-street] 通道似乎曾经用于在这一层的各种设施之间运输货物");
      ...
      DescriptionMapper.add("sector-vision", { isGroundLevel: true, buildingDensity: b23 }, "[a] [a-street]街道，两旁是摇摇欲坠的古老[a-sectortype]建筑");
      ...
      DescriptionMapper.add("sector-vision", { isSurfaceLevel: true, buildingDensity: b13, sectorType: t_P }, "一个由中央华丽的公共建筑主导的大广场");
      DescriptionMapper.add("sector-vision", { isSurfaceLevel: true, buildingDensity: b23 }, "[a] [a-street]街道，点缀着广告牌和死屏，周围是高楼");
      ...
      DescriptionMapper.add("sector-vision", { isSurfaceLevel: true, buildingDensity: b33 }, "[a] [a-street] [n-street]在高大、华丽的[n-buildings]之间");
      DescriptionMapper.add("sector-vision", { isSurfaceLevel: true, buildingDensity: b33 }, "[a] [a-street]通道在曾经的两个购物中心之间");
      DescriptionMapper.add("sector-vision", { isSurfaceLevel: true, buildingDensity: b33 }, "[a] [a-street] [n-street]，风不断地在狭窄的通道中呼啸");
      DescriptionMapper.add("sector-vision", { isSurfaceLevel: true, sectorType: t_C }, "[a] [a-street] [n-street]在曾经的两个购物中心之间");
      DescriptionMapper.add("sector-vision", { isSurfaceLevel: true, sectorType: t_C }, "一个令人印象深刻的购物中心，似乎曾满是出售奢侈品的商店");
      DescriptionMapper.add("sector-vision", { isSurfaceLevel: true, sectorType: t_I }, "[a] [a-street]与宏伟的办公楼");
      DescriptionMapper.add("sector-vision", { debris: b22 }, "[n-street]满是碎片");
      DescriptionMapper.add("sector-vision", { debris: b22, sectorType: t_R }, "[a] [n-street]两侧是几栋完全被摧毁的住宅楼");
