...
getSectorHeader:
		function (hasVision, features)
		{
			var template = " [a-street] 的 [a-sectortype] [n-street] ";
			if (features.hasCamp)
			{
		...
		function (features)
		{
			...
			switch (features.sectorType)
			{
				case SectorConstants.SECTOR_TYPE_RESIDENTIAL:
					...
					addOptions("a-street-past", ["曾经美丽", "曾经宁静", "曾经有序", "曾经令人放松"]);
					addOptions("n-building", ["居民楼", "公寓房", "有无数相同阳台的住宅楼", "住宅区"]);
					addOptions("n-buildings", ["居民楼群", "公寓群", "楼群", "小区"]);
					addOptions("an-decos", ["有轨电车轨道"]);
					addOptions("a-building", ["寂静", "有序", "巨大", "对称"]);
				...
				case SectorConstants.SECTOR_TYPE_INDUSTRIAL:
					...
					addOptions("a-building", ["停用", "有序", "巨大", "奇怪"]);
				...
				case SectorConstants.SECTOR_TYPE_COMMERCIAL:
					addOptions("n-sector", ["购物中心", "超市", "办公楼综合楼"]);
					addOptions("a-street-past", ["曾经繁华", "曾经喧嚣", "曾经充满活力"]);
				...
				case SectorConstants.SECTOR_TYPE_PUBLIC:
					...
					addOptions("a-street-past", ["休闲", "有序", "令人愉快"]);
					addOptions("n-building", ["图书馆", "监狱", "学校", "大学建筑", "公园", "公共广场", "运动场", "地铁站", "研究实验室", "政府大楼"]);
					addOptions("n-buildings", ["公共建筑", "政府办公楼"]);
				...
				case SectorConstants.SECTOR_TYPE_SLUM:
					...
					addOptions("a-street-past", ["阴暗", "曾经拥挤", "曾经热闹"]);
			...
			switch (features.condition)
			{
				case SectorConstants.SECTOR_CONDITION_RUINED:
					...
					addOptions("n-buildings", ["摇摇欲坠的废墟"]);
					addOptions("a-building", ["毁坏", "残破"]);
					break;
				case SectorConstants.SECTOR_CONDITION_DAMAGED:
					addOptions("a-street", ["损坏", "被摧毁", "残破"]);
				...
				case SectorConstants.SECTOR_CONDITION_ABANDONED:
					...
					addOptions("a-building", ["腐朽", "荒废", "缓缓风化", "早已废弃", "摇摇欲坠"]);
					break;
			...
			if (features.sunlit)
			{
				...
				if (features.wear < 5 && features.damage < 5)
					addOptions("a-street", ["闪耀", "清朗"]);
				if (features.wear > 5)
					addOptions("a-street", ["杂草丛生"]);
				addOptions("a-building", ["充满活力", "阳光照耀"]);
				addOptions("an-decos", ["顽强的小草"]);
			}
			else
			{
				addOptions("a-street", ["黑暗", "黑暗", "阴暗", "荫蔽", "暗淡"]);
			}
			// - hazards
			...
			{
				addOptions("a-street", [ "积水", "淹水" ]);
				addOptions("a-building", [ "积水" ]);
			}
			if (features.hazards.debris)
			{
				addOptions("a-street", ["被摧毁", "损坏", "毁坏"]);
				addOptions("n-building", ["建筑"]);
				addOptions("a-building", ["被摧毁", "无法辨认", "空空如也"]);
			...
			if (features.level < 6)
			{
				...
				addOptions("a-building", ["古老", "过时", "古朴", "历史性", "装饰华丽", "巴洛克风格"]);
			}
...
getPassageFoundMessage:
		function (passageVO, direction, sunlit, isBuilt)
		{
...
getPassageRepairedMessage:
		function (passageType, direction, sectorPosVO, numCampsBuilt)
		{
...
getPassageDescription:
		function (passageVO, direction, isBuilt, isShort)
		{
			let passageType = passageVO.type;
...
getPassageStatusDescription:
		function (passageVO, isBuilt)
		{
...
getReadBookMessage:
		function (itemVO, bookType, campOrdinal)
		{
			let features = {};
		...
		function (features)
		{
			...
			switch (features.bookType)
			{
				...
				case ItemConstants.bookTypes.history:
					...
					if (features.bookLevel == 1)
					{
						...
						topics.push("前大灾变时期的宗教以及它们如何导致了几场战争");
					}
					if (features.bookLevel == 2)
					{
			...
			switch (features.bookType)
			{
				case ItemConstants.bookTypes.fiction:
					...
					themes.push("前大灾变时期贫民区一个犯罪团伙的兴衰");
					...
					themes.push("前大灾变时期一家报纸办公室复杂的社会等级");
			...
			switch (features.bookType)
			{
				case ItemConstants.bookTypes.science:
					...
					break;
				case ItemConstants.bookTypes.history:
					facts.push("几个强大的采矿公司在大灾变前拥有巨大的权力");
					...
					facts.push("最后一个水下研究站在大灾变前几十年关闭");
					break;
				case ItemConstants.bookTypes.engineering:
...
getDonateSeedsMessage:
		function (itemVO)
		{
			return "将种子捐赠给了寺庙, 神职人员将珍视它们，也许会有东西生长";
		},

getReadResearchPaperMessage:
		function (itemVO)
		{
			let features = {};
			let itemName = ItemConstants.getItemDisplayName(itemVO);
...
getResearchPaperTextParams:
		function (features)
		{
			...
			facts.push("大灾变前海洋流向正在改变");
			facts.push("大灾变前几年，一些研究人员对火山活动感到担忧");
			facts.push("大灾变前有一个研究小组在调查返回地面生活的可能性");
			facts.push("城市外的空气对呼吸有害");
			facts.push("大灾变前有一个顶级秘密研究小组");
			facts.push("大灾变涉及城市表面附近的一次大爆炸");
			facts.push("大灾变前的政府在太空研究上投入巨资");
			facts.push("囚犯被用于与太空旅行相关的秘密实验");
			facts.push("政府正在编制一份紧急疏散情况下的优先个人名单");
			facts.push("大灾变前正在为某事准备的顶级秘密基因库");
...
getWaymarkText:
		function (waymarkVO, sectorFeatures)
		{
...
getWaymarkTextParams:
		function (waymarkVO, features)
		{
			let result = {};
		...
		function (waymarkVO)
		{
			switch (waymarkVO.type)
			{
				...
				case SectorConstants.WAYMARK_TYPE_POLLUTION:
					return "危险";
				case SectorConstants.WAYMARK_TYPE_SETTLEMENT:
					return "贸易点";
				default:
...
getHeapDisplayName:
		function (resourceName, features)
		{
...
createTextFromLogMessage:
		function (msg, replacements, values, includePeriod)
		{
			var text = msg;
			var value = 0;
...
getLocaleName:
		function (locale, sectorFeatures, isShort)
		{
...
getWorkshopName:
		function (resource)
		{
			switch (resource)
			{
				case resourceNames.fuel:
					return "炼油厂";
				case resourceNames.rubber:
					return "种植园";
				default:
					return "工坊";
			}
...
getEnemyText:
		function (enemyList, sectorControlComponent)
		{
			let result = "";
...
getEnemyNoun:
		function (enemyList, detailed, pluralify)
		{
...
getEnemeyDefeatedVerb:
		function (enemyList)
		{
			return this.getCommonText(enemyList, "defeatedV", "", "击败", false);
		},

getScaResourcesString:
		function (discoveredResources, knownResources, resourcesScavengable)
		{
			var s = "";
			for (var key in resourceNames)
			{
				...
				{
					var amountDesc = "较少";
					if (amount == WorldConstants.resourcePrevalence.RARE) amountDesc = "很少";
					if (amount == WorldConstants.resourcePrevalence.DEFAULT) amountDesc = "较少";
					if (amount == WorldConstants.resourcePrevalence.COMMON) amountDesc = "常见";
					if (amount == WorldConstants.resourcePrevalence.ABUNDANT) amountDesc = "许多";
...
getScaItemString:
		function (discoveredItems, knownItems, itemsScavengeable)
		{
			let validItems = [];
...
getMovementBlockerName:
		function (blockerVO, enemiesComponent, gangComponent)
		{
...
getMovementBlockerAction:
		function (blockerVO, enemiesComponent, gangComponent)
		{
			switch (blockerVO.type)
			{
				...
				case MovementConstants.BLOCKER_TYPE_GANG:
					let enemies = this.getAllEnemies(null, gangComponent);
					return "和" + this.getEnemyNoun(enemies, false, true) + "战斗";
			}
		},

getAllEnemies:
		function (enemiesComponent, gangComponent)
		{
			let enemies = [];
...
getCommonText:
		function (objectList, objectAttribute, objectDetailAttribute, defaultWord, allowSeveral, pluralify)
		{
			var allWords = [];
			var allDetails = [];
			var minimumWords = [];
			...
			{
				return pluralify ? (Text.pluralify(minimumWords[0]) + "和" + Text.pluralify(minimumWords[1])) : (minimumWords[0] + " and " + minimumWords[1]);
			}
			else
			{
				return defaultWord;
...
getListText:
	...
	function initSectorTexts()
	{
		...
		DescriptionMapper.add("sector-vision", { sectorType: wildcard }, "有一些 [an-decos] 的一个 [n-street] 和 [a-building] 的 [n-buildings] ");
		...
		DescriptionMapper.add("sector-vision", { wear: b13, level: lmodern, isSurfaceLevel: false }, "一条 [n-street] 位于一些还在建设中就被遗弃的建筑骨架之间");
		...
		DescriptionMapper.add("sector-vision", { sectorType: t_C }, "一条 [a-street] 的购物街，遗留着各种商店和咖啡馆的残骸");
		DescriptionMapper.add("sector-vision", { sectorType: t_C }, "一条 [n-street] ，位于一些建筑之间，它们的 [a-building] 墙面覆盖着死屏的拼接");
		DescriptionMapper.add("sector-vision", { sectorType: t_C, wear: b12 }, "一条 [a-street] 的 [n-street] 塞满了小商店、广告牌和许多层的小亭子");
		...
		DescriptionMapper.add("sector-vision", { sectorType: t_C, buildingDensity: b22, wear: b33 }, "一个 [a-building] 建筑，很难确定其原始用途，现在只剩混凝土了，中间有一个令人印象深刻的螺旋楼梯");
		...
		DescriptionMapper.add("sector-vision", { level: 14, buildingDensity: b13 }, "一个巨大的大厅，看起来曾经被用作某种存储区域，天花板上有生锈的机器手臂");
		...
		DescriptionMapper.add("sector-vision", { level: 14, buildingDensity: b33 }, "一个 [a-street] 的通道似乎曾经用于在这一层的各种设施之间运输货物");
		...
		DescriptionMapper.add("sector-vision", { isSurfaceLevel: true, buildingDensity: b23 }, "一个 [a-street] 街道，点缀着广告牌和坏掉的屏幕，周围是高楼");
		...
		DescriptionMapper.add("sector-novision", { sunlit: false, buildingDensity: b0 }, "城市内罕见的空地；没有地板或墙壁，没有建筑，什么也没有只有广阔的空虚黑暗");
		DescriptionMapper.add("sector-novision", { sunlit: false, buildingDensity: b13 }, "一条宽阔的街道或走廊在浩瀚的黑暗中很难找到任何东西");
		DescriptionMapper.add("sector-novision", { sunlit: false, buildingDensity: b23, wear: b22 }, "一条走廊或带有被遗弃气息的街道黑暗中看不清细节");
		DescriptionMapper.add("sector-novision", { sunlit: false, buildingDensity: b23, wear: b12 }, "一条安静的街道或走廊黑暗中看不清细节");
		DescriptionMapper.add("sector-novision", { sunlit: false, buildingDensity: b33 }, "一个密集的通道，几乎没有足够的空间行走你在黑暗中摸索前行");
		DescriptionMapper.add("sector-novision", { sunlit: false }, "城市内隐藏在黑暗中的空间");
		DescriptionMapper.add("sector-novision", { sunlit: true, buildingDensity: b0 }, "城市内罕见的空地；没有地板或墙壁，没有建筑，什么也没有只有广阔的空虚");
		DescriptionMapper.add("sector-novision", { sunlit: true, buildingDensity: b13 }, "一条宽阔的街道或走廊在刺眼的阳光下很难找到任何东西");
		DescriptionMapper.add("sector-novision", { sunlit: true, buildingDensity: b23, wear: b22 }, "一条走廊或带有被遗弃气息的街道刺眼的光线中看不清细节");
		DescriptionMapper.add("sector-novision", { sunlit: true, buildingDensity: b23, wear: b12 }, "一条安静的街道或走廊刺眼的光线中看不清细节");
		DescriptionMapper.add("sector-novision", { sunlit: true, buildingDensity: b33 }, "一个密集的通道，几乎没有足够的空间行走你在耀眼的光线中摸索前行");
	...
	function initWaymarkTexts()
	{
		...
		DescriptionMapper.add("waymark", { sectorType: t_I }, "一个带有方向的街道标志被涂改朝向 [direction] 的地方写着 [n-target] ");
		DescriptionMapper.add("waymark", { sectorType: t_M }, "天花板附近的管道上画有箭头指向 [direction] 的旁边是 [n-target] 的符号");
	...
	function initBookTexts()
	{
		...
		DescriptionMapper.add("book-description", { bookType: t_S, bookLevel: l_1 }, "一篇关于精炼过程的描述提供了有关大灾变前常用建筑材料的线索");
		DescriptionMapper.add("book-description", { bookType: t_S, bookLevel: l_1 }, "它包含了一个已知的‘暗层’动物生活的目录你认出了几个");
		...
		DescriptionMapper.add("book-description", { bookType: t_H }, "一个关于 [c-event] 的描述吸引了你的兴趣");
		...
		DescriptionMapper.add("book-description", { bookType: t_H }, "提到“当前无人居住的层次”的参考为大灾变前的城市提供了一个视角");
