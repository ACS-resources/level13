...
function (Ash, GameGlobals, ItemConstants, PerkConstants, LocaleConstants, PositionConstants, UpgradeConstants, WorldConstants, ResourcesVO) {
	...
	var FightConstants = {
		...
		getPlayerAttDesc: function (playerStamina, itemsComponent, explorersComponent) {
			...
			var explorerBonus = explorersComponent.getCurrentBonus(ItemConstants.itemBonusTypes.fight_att);
			var desc = "";
			if (itemBonus <= 0) desc += "角色: " + this.FIGHT_PLAYER_BASE_ATT;
			if (itemBonus > 0) desc += "装备: " + itemBonus;
			if (healthFactor < 1) desc += "<br/>生命值: -" + Math.round((1-healthFactor) * 1000) / 10 + "%";
			if (explorerBonus > 0) desc += "<br/>探索者: " + explorerBonus;
			return desc;
		},
		...
		getPlayerDefDesc: function (playerStamina, itemsComponent, explorersComponent) {
			...
			let desc = "";
			if (itemBonus > 0) desc += "装备: " + itemBonus;
			else desc += "角色: " + this.FIGHT_PLAYER_BASE_DEF;
			var explorerBonus = explorersComponent.getCurrentBonus(ItemConstants.itemBonusTypes.fight_def);
			if (explorerBonus > 0) desc += "<br/>探索者: " + explorerBonus;
			return desc;
		},
		...
		getPlayerShieldDesc: function (playerStamina, itemsComponent) {
			return "装备: " + itemsComponent.getCurrentBonus(ItemConstants.itemBonusTypes.fight_shield);
		},
		...
		getTurnScenarios: function (participantType, enemy, playerStamina, itemsComponent, explorersComponent, fightTime) {
			...
			let participantName = participantType == FightConstants.PARTICIPANT_TYPE_FRIENDLY ? "角色" : "敌人";
